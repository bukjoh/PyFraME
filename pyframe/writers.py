...
class InputWriters(object):
    ...
    def molcas_multipoles_polarizability(fragment, region, core_region, filename=None):
        ...
        inp = '&GATEWAY\n'
        inp += f'TITLE = {get_version_string()}\n'
        inp += 'COORD = {0}.xyz\n'.format(filename)
        inp += 'BASIS = {0}\n'.format(region.basis)
        inp += 'GROUP = C1\n'
        inp += '&SEWARD\n'
    ...
    def xyz(elements, coordinates, charge, filename):
        """Write XYZ file"""
        xyz = '{0}\n'.format(len(elements))
        xyz += f'{get_version_string()} (fragment charge: {charge})\n'
            ...
            if incoming_unit == "Angstrom":
                ...
                for fragment in system.core_region.fragments.values():
                    for atom in fragment.atoms:
                        ...
                        sys_dict["quantum_subsystems"][0]["nuclei"].append(nucleus)
                        k += 1
                ...
                for fragment in system.core_region.fragments.values():
                    for atom in fragment.atoms:
                        ...
                        sys_dict["quantum_subsystems"][0]["nuclei"].append(nucleus)
                        k += 1
        # fill classical_subsystems with atoms
        sys_dict['classical_subsystems'] = [{"classical_fragments": [{"name": "all atoms",
                                                                      "index": 0,
                                                                      "atoms": []}]
                                             }]
        multipoles = []
