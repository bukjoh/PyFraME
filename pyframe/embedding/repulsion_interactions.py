...
def compute_repulsion_interactions_hessian(quantum_subsystem: subsystem.QuantumSubsystem,
                                           classical_subsystem: subsystem.ClassicalSubsystem,
                                           method: str = 'LJ',
                                           combination_rule: str = 'Lorentz-Berthelot'
                                           ) -> np.ndarray:
    ...
    if method == 'LJ':
        ...
        if combination_rule == 'Lorentz-Berthelot':
            engine.set_combination_rule(combination_rule)
            if comm is None:
                rep_hessian_contr = engine.lj_repulsion_hessian(np.array([0, len(classical_subsystem.coordinates)],
                                                                           dtype=np.int64))
            else:
                rank = comm.Get_rank()
                size = comm.Get_size()
                ...
                rep_hessian_contr = engine.lj_repulsion_hessian(np.array([start, end], dtype=np.int64))
                rep_hessian_contr = classical_subsystem.comm.allreduce(rep_hessian_contr)
            return rep_hessian_contr
        else:
            raise NotImplementedError("This combination rule has not been implemented yet.")
    else:
        raise NotImplementedError("This method has not been implemented yet.")
