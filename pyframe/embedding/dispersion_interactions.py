...
def compute_dispersion_interactions_hessian(quantum_subsystem: subsystem.QuantumSubsystem,
                                            classical_subsystem: subsystem.ClassicalSubsystem,
                                            method: str = 'LJ',
                                            combination_rule: str = 'Lorentz-Berthelot'
                                            ) -> np.ndarray:
    """Computes the dispersion (London dispersion) Hessian of the Nuclei of a QuantumSubsystem interacting
    with a ClassicalSubsystem.
    ...
    if method == 'LJ':
        ...
        if combination_rule == 'Lorentz-Berthelot':
            engine.set_combination_rule(combination_rule)
            if comm is None:
                disp_hessian_contr = engine.lj_dispersion_hessian(np.array([0, len(classical_subsystem.coordinates)],
                                                                           dtype=np.int64))
            else:
                rank = comm.Get_rank()
                size = comm.Get_size()
                ...
                disp_hessian_contr = engine.lj_dispersion_hessian(np.array([start, end], dtype=np.int64))
                disp_hessian_contr = classical_subsystem.comm.allreduce(disp_hessian_contr)
            return disp_hessian_contr
        else:
            raise NotImplementedError("This combination rule has not been implemented yet.")
    else:
        raise NotImplementedError("This method has not been implemented yet.")
