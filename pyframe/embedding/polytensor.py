...
from pyframe.embedding import tensor_tools, engine
...
class PolytensorStatics:
    """A statics object represents a collection of variables.
    """
    initialized = False
    max_order = None
    ...
    def __init__(self,
        ...
        if not self.initialized:
            ...
            t_int_obj = SecondDegreePolytensor(rank_1=t_rank, rank_2=t_rank, data_type=object)
            t_int_obj.write_interaction_tensor_multi_indices()
            t_pot_obj = SecondDegreePolytensor(rank_1=t_rank, rank_2=t_rank, data_type=object)
            ...
            self.trinomials = tensor_tools.compute_trinomial_coefficients(max_order, max_order, max_order,
                                                                          self.binomials)
            self.degeneracies = FirstDegreePolytensor(self.max_order)
