...
def test_unrecognized_input():
    ...
    with pytest.raises(TypeError, match=re.escape('Argument "input_data" must be of type "dict", "Path", or "str" '
                                                  '(not "int").')):
...
def test_json_to_dict(sample_json):
    ...
    assert "quantum_subsystems" in data
    assert "classical_subsystems" in data
...
def test_missing_subsystems():
    # Test if KeyError is raised when quantum_subsystem key is missing
    input_data = f'{os.path.dirname(__file__)}/data/no_subsystems.json'
    with pytest.raises(KeyError, match="No Subsystems could be read in."):
