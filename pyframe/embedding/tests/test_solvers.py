...
def test_induced_dipoles_jacobi(
    ...
    electric_field = -1.0 * act_wat_electric_fields
    nuclear_field = -1.0 * np.array([[0.00392044, -0.05293017, -0.10427627],
...
def test_induced_dipoles_jacobi_large_inputs():
    ...
    core, env = read_input.reader(input_data=f'{os.path.dirname(__file__)}/data/act_wat_mid.json')
    env.solve_induced_dipoles(threshold=1e-8)
    ref_ind_dip = -1.0 * np.array([[-5.33083758e-03, 1.80899242e-02, -4.67124873e-02],
